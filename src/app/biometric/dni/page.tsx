...
function cleanError(raw?: string): string {
  ...
  let t = raw
    ...
    .replace(/\?/g, "ó");
  t = t
    .replace(/Validaci\?n/gi, "Validación")
...
function generateDetailedErrorMessage(resp: any): string {
  if (!resp?.response?.json) return "Error al procesar la validación.";
  ...
  if (failedValidations.length === 0) return "Validación exitosa";

  const messages: string[] = [];
  const faceCompare = validations.find((v: any) => v.test === "FACE_COMPARE");
  const faceDetection = validations.find(
    (v: any) => v.test === "FACE_DETECTION"
  );
  const documentDetection = validations.find(
    (v: any) => v.test === "DOCUMENT_DETECTION"
  );
  const documentQuality = validations.find(
    (v: any) => v.test === "DOCUMENT_QUALITY"
  );
...
function ClientContent() {
  ...
  const [step, setStep] = useState<
    "intro" | "face" | "face-preview" | "dni" | "dni-preview" | "result"
  >("intro");
  ...
  const [res04, setRes04] = useState<{
    success: boolean;
    message: string;
    detail?: Validate04Detail;
  } | null>(null);
  ...
  async function send04() {
    ...
    try {
      ...
      const resp: Validate04Detail = await validateDNIWithPhoto04({
        photoFace,
        photoDNI,
        email,
        f1: f1 || "f1",
      ...
      setRes04({
        success: false,
        message: cleanError(e?.message) || "Error de red o servidor",
      });
    } finally {
      setLoading(false);
      setStep("result");
  ...
  if (step === "face") {
    return (
      <CameraScreen
        title="Captura una foto de tu rostro"
        subtitle='Alinea tu rostro dentro del recuadro y presiona “Tomar foto”.'
        onCapture={(data) => {
          setPhotoFace(data);
  ...
  if (step === "face-preview" && photoFace) {
    return (
      <CameraPreview
        title="Validación DNI con selfie!"
        subtitle="Paso 2 de 3: Revisa tu selfie"
        imageSrc={photoFace}
  ...
  // ✅ NUEVA CONFIGURACIÓN PARA FOTO DEL DNI
  if (step === "dni") {
    return (
      <CameraScreen
        title="Captura una foto de tu DNI"
        subtitle='Alinea tu DNI dentro del recuadro y presiona “Tomar foto”.'
  ...
  if (step === "result") {
    ...
    const detail = (res04?.detail?.response || {}) as NonNullable<
      Validate04Detail["response"]
    >;
    const json: {
      overallStatus?: boolean;
      validations?: Array<{ test: string; status: boolean; result: string }>;
    ...
    const validations = Array.isArray(json?.validations)
      ? json.validations
      : [];
